// this code control the robot and the action should be used to sequential method of isaac sim simulation
...
#include <unistd.h>
void move_to_initial_with_tcp(franka::Robot& robot, MotionGenerator& motion_generator, int client_socket, int& message_count) {
                robot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::JointPositions {
                    // Update robot's position
                    franka::JointPositions output = motion_generator(rstate, periode);

                    // Construct message to send over TCP
                    std::string test = "{\"q\": [";
                        for (int i = 0;i<9;i++){
                        if (i<7){
                    ...
                    test += "]}\r\n";
                    std::string tcp_message = test;

                    // Send TCP data
                    send(client_socket, tcp_message.c_str(), tcp_message.length(), MSG_DONTWAIT);
                    std::cout << "TCP Message sent: " << tcp_message << std::endl;
...
auto cartesian_motion = [](std::array<double, 16>& initial_pos, double& time, double finish_time, double gw,
                           const std::array<double, 3>& displacement, int client_socket, int& message_count,
                           const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
    {
        time += periode.toSec();
        ...
        current_pos[12] += displacement[0] * (std::cos(M_PI_4 * (1 - std::cos(M_PI / finish_time * time))) -1);
        current_pos[13] += displacement[1] * (std::cos(M_PI_4 * (1 - std::cos(M_PI / finish_time * time))) -1);
        current_pos[14] += displacement[2] * (std::cos(M_PI_4 * (1 - std::cos(M_PI / finish_time * time))) -1);
        ...
        std::cout << "send message to client: " << test.c_str() << " Length: " << test.length() << std::endl;
...
//         // htons converts from host to network byteorder
           address.sin_port = htons(8081); //  port from 8080 to 8081, otherwise no client socket can be created
...
//           double time0 = 0.0;
//           double finish_time0 = 1.0;
//           state = gripper.readOnce();
//           double GW0 = state.width;
...
//          auto cartesian_initial_motion_0 = [&initial_pos0, &time0, &finish_time0, &client_socket, &GW0](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//         {
...
//          auto cartesian_initial_motion_1 = [&initial_pos1, &time1, &finish_time1, &client_socket,&message_count,&GW1](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//         {
...
//              double dx = -0.16 * (cos(M_PI_4*(1-cos(M_PI/3*time1)))-1);
//              double dy = 0.26 * (cos(M_PI_4*(1-cos(M_PI/3*time1)))-1);
//              double dz = 0* (cos(M_PI_4*(1-cos(M_PI/3*time1)))-1);
...
//            double time11 = 0.0;
//           double finish_time11 = 3.0;
//           state = gripper.readOnce();
//           double GW11 = state.width;

// //         // at the initial position: echo_robot_state 192.168.3.100
// //         // find the variable "O_T_EE"
// //         // this is the initial_pos1 
//           std::array<double, 16> initial_pos11; 
        

//          auto cartesian_initial_motion_11 = [&initial_pos11, &time11, &finish_time11, &client_socket,&message_count,&GW11](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//         {
//             std::cout <<rstate.O_T_EE_c[14] << std::endl;
//             time11 += periode.toSec();

//             if(time11==0.0)
//             {
//                 initial_pos11 = rstate.O_T_EE_c;
//                 // Send go for recoding to client app
//                 send(client_socket, "GO", sizeof("GO"), MSG_DONTWAIT);
//             }
...
//             //double dz = -0.1*(cos(M_PI_4*(1-cos(M_PI/5*time1))));
//               constexpr double kRadius = 0.4;
//              double angle = M_PI / 4 * (1 - std::cos(M_PI / 5.0 * time11));
//              double dx = 0 * (cos(M_PI_4*(1-cos(M_PI/3*time11)))-1);
//              double dy = 0 * (cos(M_PI_4*(1-cos(M_PI/3*time11)))-1);
//              double dz = 0.4* (cos(M_PI_4*(1-cos(M_PI/3*time11)))-1);
//             std::array<double, 16> current_pos = initial_pos11;
//             current_pos[12] += dx;
//             current_pos[13] += dy;
//             current_pos[14] += dz;
...
//            myRobot.control(cartesian_initial_motion_11);
...
// //         // 3. Move up
//           std::cout << "2.2 gripper grasping"  << client_socket << std::endl;
//          double time_22 = 0.0;
//          double finish_time_22 = 3.0;
//         //  state = gripper.readOnce();
//         //  double  GW2 = state.width;

//          std::array<double, 16> initial_pos_22;

//          auto grasping = [&initial_pos_22, &time_22, &finish_time_22,&client_socket,&message_count](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//         {

//             time_22 += periode.toSec();


//             if(time_22==0.0)
//             {
//                 initial_pos_22 = rstate.O_T_EE_c;
//             }
...
//                std::array<double, 16> current_pos = initial_pos_22;
// //             current_pos[12] += dx;
// //             current_pos[13] += dy;
//                //current_pos[14] += dz;
...
//                 std::cout << "t:" << time_22 << "  periode: " << periode.toSec() << " z-Koord.: " << rstate.O_T_EE_c[14] << std::endl;
...
//          auto cartesian_moving_slightlyUp = [&initial_pos_2, &time_2, &finish_time_2,&client_socket,&message_count,&GW2](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//         {
...
//             state = gripper.readOnce();
//            double GW3 = state.width;

//            std::array<double, 16> initial_pos_3; 

//             auto cartesian_moving_around = [&initial_pos_3, &time_3, &finish_time_3,&client_socket,&message_count,&GW3](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//             {
...
//                 double dx = 0.15 * (cos(M_PI_4*(1-cos(M_PI/3*time_3)))-1);
...
//             state = gripper.readOnce();
//            double GW32 = state.width;
//            std::array<double, 16> initial_pos_32; 

//             auto cartesian_moving_around2 = [&initial_pos_32, &time_32, &finish_time_32,&client_socket,&message_count, &GW32](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//             {
...
//                  double dx = 0.45 * (cos(M_PI_4*(1-cos(M_PI/4*time_32)))-1);
...
//             state = gripper.readOnce();
//            double GW33 = state.width;
//            std::array<double, 16> initial_pos_33; 

//             auto cartesian_moving_around3 = [&initial_pos_33, &time_33, &finish_time_33,&client_socket,&message_count,&GW33](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose
//             {
...
//                  double dx = 0.455 * (cos(M_PI_4*(1-cos(M_PI/4*time_33)))-1);
...
//             state = gripper.readOnce();
//            double GW4 = state.width;
//           std::array<double, 16> initial_pos_4;

//         auto cartesian_moving_slightlyDown = [&initial_pos_4, &time_4, &finish_time_4,&client_socket,&message_count, &GW4](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose{
...
// // Function to move robot back to initial position and send data via TCP
            

//          std::cout << "8. To Initial"  << std::endl;
//          move_to_initial_with_tcp(myRobot, motiongenerator, client_socket, message_count);
...
//          std::this_thread::sleep_for(std::chrono::seconds(5));
            ...
            double time9 = 0.0;
            double finish_time9 = 5.0;
            double gw9 = 0.03;
            std::array<double, 16> initial_pos9;
            std::array<double, 3> displacement9 = {-0.205, -0.195, 0.28}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos9, time9, finish_time9, gw9, displacement9, client_socket, message_count, rstate, periode);
            });
            

            double time10 = 0.0;
            double finish_time10 = 1.0;
            //double gw9 = 0.03;
            std::array<double, 16> initial_pos10;
            std::array<double, 3> displacement10 = {0, 0, 0.02}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos10, time10, finish_time10, gw9, displacement10, client_socket, message_count, rstate, periode);
            });
            gripper.grasp(0.01, 0.02, 70);

            double time111 = 0.0;
            double finish_time111 = 2.0;
            double gw11 = 0.02;
            std::array<double, 16> initial_pos111;
            std::array<double, 3> displacement11 = {0, 0, -0.15}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos111, time111, finish_time111, gw11, displacement11, client_socket, message_count, rstate, periode);
            });
          ...
          // 
            double time12 = 0.0;
            double finish_time12 = 3.0;
            double gw12 = 0.02;
            std::array<double, 16> initial_pos12;
            std::array<double, 3> displacement12 = {0.205, 0.195, -0.1}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos12, time12, finish_time12, gw12, displacement12, client_socket, message_count, rstate, periode);
            });


            //           // put back the bottle to the storage cell
            double time13 = 0.0;
            double finish_time13 = 3.0;
            double gw13 = 0.02;
            std::array<double, 16> initial_pos13;
            std::array<double, 3> displacement13 = {-0.16, 0.26, 0.3}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos13, time13, finish_time13, gw13, displacement13, client_socket, message_count, rstate, periode);
            });
            double time14 = 0.0;
            double finish_time14 = 1.0;
            double gw14 = 0.02;
            std::array<double, 16> initial_pos14;
            std::array<double, 3> displacement14 = {0, 0, 0.045}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos14, time14, finish_time14, gw14, displacement14, client_socket, message_count, rstate, periode);
            });
             ...
             gripper.stop();
            double time15 = 0.0;
            double finish_time15 = 3.0;
            double gw15 = 0.03;
            std::array<double, 16> initial_pos15;
            std::array<double, 3> displacement15 = {0, 0, -0.2}; // 

            myRobot.control([&](const franka::RobotState& rstate, franka::Duration periode) -> franka::CartesianPose {
                return cartesian_motion(initial_pos15, time15, finish_time15, gw15, displacement15, client_socket, message_count, rstate, periode);
            });
