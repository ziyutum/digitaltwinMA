...
class PandaAIS(BaseSample):
    def __init__(self) -> None:
        ...
        self.stopper_initial_position = None  
        self.time_step = 0.0  # 
        self.velocity = 0.01  # 1cm/s 
        self.max_movement = 0.1 
        self.stopper_initial_position_l = None
        self.stopper_initial_position_r = None  
        self.move_count = 0  # for control the movement of the stopper
        self.max_moves = 50 # max number of movements loop of the stopper
        self.rightincreasing = True 
        self.leftincreasing= False   # current direction of the stopper, increasing or decreasing the y position plus or minus
    ...
    def setup_scene(self):
        ...
        panda_robot = world.scene.add(Robot(prim_path="/World/franka/factory_franka", name="panda_robot", 
                                     scale =[1],
                                     position=np.array([0.285,-0.70,0.79]),
        #                             # orientation = [1,0,0,-1]
                                    ))
        ...
        absolute_asset_pathstopper = "/home/masais/panda/Franka/stopper.usd"
        # # This will create a new XFormPrim and point it to the usd file as a reference
        # # Similar to how pointers work in memory
        add_reference_to_stage(usd_path=absolute_asset_pathstopper, prim_path="/World/stopperleft")
        

        stopperleft = world.scene.add(RigidPrim(prim_path="/World/stopperleft/stopper", name="stopperleft", 
                                     scale =[0.001,0.001,0.001],
                                     position=[1.165,-1.3028,0.79715],
        #                             # orientation = [1,0,0,-1]
                                    ))
        
        add_reference_to_stage(usd_path=absolute_asset_pathstopper, prim_path="/World/stopperright")
        

        stopperright = world.scene.add(RigidPrim(prim_path="/World/stopperright/stopper", name="stopperright", 
                                     scale =[0.001,0.001,0.001],
                                     position=[1.184,-1.2586,0.79715],
        #                             # orientation = [1,0,0,-1]
                                    ))
    ...
    async def setup_post_load(self):
        ...
        self._stopperleft = self._world.scene.get_object("stopperleft")
        self._stopperright = self._world.scene.get_object("stopperright")
    ...
    def send_robot_actions(self, step_size): 
        ...
        #----------------------------------------1022test--------.------------------------------------------------------------------------------------------------------------------------------
        # IT IS NOT WORKING WHEN I TRY TO MOVE THE STOPPER FROM THE DATA RECEIVED FROM RCP
        #BUT IT WORKS WHEN I DIRECTLY MOVE THE STOPPER PRESSING THE START BUTTONS IN THE SIMULATION
        # print("Stopper begins to move")
        # stage = omni.usd.get_context().get_stage()
        # stopper_path_right = "/World/myJoghurt/myJoghurt_ZIyu/Geometry/VN3066_000_Layout_Uni_Kassel_070919_0/VN3066_054_Stopper_Vereinzeler_2_3826/_00691290_SPANNBACKEN_D_MPS_PA_E_SP_2_3893"
        # stopper_prim_r = stage.GetPrimAtPath(stopper_path_right)
        # xformable_r = UsdGeom.Xformable(stopper_prim_r)
        # stopper_path_left = "/World/myJoghurt/myJoghurt_ZIyu/Geometry/VN3066_000_Layout_Uni_Kassel_070919_0/VN3066_054_Stopper_Vereinzeler_2_3826/_00691290_SPANNBACKEN_D_MPS_PA_E_SP_3894"
        # stopper_prim_l = stage.GetPrimAtPath(stopper_path_left)
        # xformable_l= UsdGeom.Xformable(stopper_prim_l)
        # transform_matrix_l= xformable_l.ComputeLocalToWorldTransform(0)
        # transform_matrix_r= xformable_r.ComputeLocalToWorldTransform(0)
        # stopper__position_l = transform_matrix_l.ExtractTranslation()
        # stopper__position_r = transform_matrix_r.ExtractTranslation()
        # if self.stopper_initial_position_l is None:
        #     self.stopper_initial_position_l = stopper__position_l
        # if self.stopper_initial_position_r is None:
        #     self.stopper_initial_position_r = stopper__position_r
        
        # # Initialize the initial position if it hasn't been set
        
        # if self.rightincreasing:
        #     if self.move_count < self.max_moves:
        #         set_or_add_translate_op(xformable_r, increasing=True)
        #         set_or_add_translate_op(xformable_l, increasing=False)
        #         self.move_count += 1  # 
        #         print(f"Moving Up: Move Count = {self.move_count}")
        #     else:
        #         # begin to decrease
        #         self.rightincreasing = False
        #         self.move_count += -1 
        #         print("Switching to decreasing mode.")
        # else:
        #     # decreasing mode
        #     if self.move_count > 0:  # Use move_count to control stopping
        #         set_or_add_translate_op(xformable_r, increasing=False)
        #         set_or_add_translate_op(xformable_l, increasing= True)  # Only update position
        #         self.move_count -= 1  # Increment move count
        #         print("Moving Down")
        #     else:
        #         print("Reached maximum decrease moves. Stopping movement.")
        #         self.rightincreasing = False
        ...
        # #----------------------------------------------------------------------------------------------------------------------------------
        #  # Compute the current position of the stopper
        # transform_matrix = xformable.ComputeLocalToWorldTransform(0)
        # stopper__position = transform_matrix.ExtractTranslation()
        # print("Stopper position: ", stopper__position)
        # new_position = Gf.Vec3d(stopper__position[0], stopper__position[1], stopper__position[2])
        # print("New Stopper Position: ", new_position)
        # # Initialize the initial position if it hasn't been set
        # if self.stopper_initial_position is None:
        #     self.stopper_initial_position = stopper__position
        #  # 
        # if self.increasing:
        #     if self.move_count < self.max_moves:
        #         set_or_add_translate_op(xformable)
        #         self.move_count += 1  # 
        #         print(f"Moving Up: Move Count = {self.move_count}")
        #     else:
        #         # begin to decrease
        ...
        #     # decreasing mode
        #     if self.move_count > 0:  # Use move_count to control stopping
        #         set_or_add_translate_op(xformable)  # Only update position
        #         self.move_count -= 1  # Increment move count
        #         print("Moving Down")
        #     else:
        #         print("Reached maximum decrease moves. Stopping movement.")
        ...
        try:
            ...
            #     self._franka.set_joint_positions(nparray_new)
                ...
                # stopper_prim_l = stage.GetPrimAtPath(stopper_path_left)
                # print( 1)
                # # rigid_body_api_r = UsdPhysics.RigidBodyAPI.Apply(stopper_prim_r)
                # # rigid_body_api_l = UsdPhysics.RigidBodyAPI.Apply(stopper_prim_l)
                # print( 2)
                # # if UsdPhysics.CollisionAPI.Has(stopper_prim_r):
                # #     UsdPhysics.CollisionAPI(stopper_prim_r).GetPrim().RemoveAPI(UsdPhysics.CollisionAPI)
                # #     print( 3)

                # # if UsdPhysics.CollisionAPI.Has(stopper_prim_l):
                # #     UsdPhysics.CollisionAPI(stopper_prim_l).GetPrim().RemoveAPI(UsdPhysics.CollisionAPI)


                # xformable_l= UsdGeom.Xformable(stopper_prim_l)
                # print( 2.5)
                # if not stopper_prim_r.IsValid():
                #     print(f"Invalid right stopper path: {stopper_path_right}")
                # if not stopper_prim_l.IsValid():
                #     print(f"Invalid left stopper path: {stopper_path_left}")
                ...
                #     print(f"Error extracting stopper position: {e}")
