/* 
this code is for creating new relationships under nodes when uploading the azure data.
 
 */
// new version 26,08,2024
using System;
...
namespace CSClient
{
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            string srcIdtransport = "TransportProducts";
            string srcIdpick = "PickupAndPlace";
            string targetId = "Bottle_Position";
            string relName = "value";
            ...
            // ziyu change the method asyn to normal
            try
            {
                string relId = $"{srcIdtransport}-{relName}->{targetId}";
                string relIdnew = $"{srcIdpick}-{relName}->{targetId}";
                m_azureClient.DeleteRelationship(srcIdtransport, relId);
                m_azureClient.CreateOrReplaceRelationship<BasicRelationship>(srcIdpick, relIdnew, relationship);
                Console.WriteLine($"Created {relName} relationship successfully. Relationship ID is {relIdnew}.");
            }
            catch (RequestFailedException ex)
            {
                Console.WriteLine($"Error creating relationship. {ex.Status}: {ex.Message}");
            }
        ...
        static void process_and_upload_fci_data()
        {
            ...
            int necessary_buffer_size = 25000;//average_package_size*feasible_read_interval_time;
            ...
            Timer timerMaxRead = new System.Timers.Timer(100000); // Timer for whole reading process [ms]
            ...
            void readData(Object source, ElapsedEventArgs e)
            {
                ...
                {   
                    ...
                    if (funcCount % 80 == 0)
                    {
                        ...
                        // string srcId = "Status";
                        // string targetId = "Pick_and_Place_JoghurtProducts";
